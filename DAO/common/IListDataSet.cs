...
namespace DAO.common
{
    public class IListDataSet
    {
        ...
        /// DataSet装换为泛型集合 
        /// </summary> 
        /// <typeparam name="T"></typeparam> 		p_Data.Rows[j][i]	4	object {ulong}
                ...
                foreach (PropertyInfo pi in propertys)
                {
                    for (int i = 0; i < p_Data.Columns.Count; i++)
                    {
                        ...
                        if (pi.Name.Equals(p_Data.Columns[i].ColumnName))
                        {
                            ...
                            {
                                if (p_Data.Rows[j][i].GetType().ToString() == "MySql.Data.Types.MySqlDateTime")
                                {
                                    pi.SetValue(_t, ((MySql.Data.Types.MySqlDateTime)p_Data.Rows[j][i]).GetDateTime(), null);
                                }
                                else
                                {
